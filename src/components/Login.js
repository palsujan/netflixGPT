  ...
  return (
    <div>
        ...
        <h1 className='font-bold text-3xl py-4'>{isSignInForm ? "Sign In" : "Sign Up"}</h1>
           {!isSignInForm && <input type='text' placeholder='Enter your Name' className='p-2 m-2 w-full bg-gray-800'/>}
            ...
            <button className='py-4 mx-2 my-6 bg-red-700 w-full rounded-lg'>{isSignInForm ? "Sign In" : "Sign Up"}</button>
            <p className='py-4 cursor-pointer' onClick={toggleSignInForm}>{isSignInForm ? "Sign Up Now " : "Allready Registerd ? Sign In Now"}</p>
        </form>
    </div>
  )
}
